...
export default function LandingPage() {
  useEffect(() => {
    ...
    if (path1 && path2 && path3) {
      ...
      gsap.to(path1, { strokeDashoffset: 0, duration: 2, ease: "power2.out", onComplete: () => {
      ...
      gsap.to(path2, { strokeDashoffset: 0, duration: 2, ease: "power2.out", onComplete: () => {
      ...
      gsap.to(path3, { strokeDashoffset: 0, duration: 2, ease: "power2.out", onComplete: () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center bg-black text-white">
      ...
      <div className="mb-10">
        ...
        >
          <path 
            ...
            stroke="white" 
            strokeLinecap="round"
            fill="white" // Add initial fill
            fillOpacity="0" // Hide fill initially
          />
          <path 
            id="logoS"
      ...
      <h2 className="text-3xl font-bold text-left justify-left py-10">About Us</h2>
      <p className="text-xl text-left justify-left max-w-screen px-20">
      We, <i>The Computer Science Association</i> are more than just an association; we're the energy behind the CSIS Department at BITS Pilani, Hyderabad Campus, the guardians of all things geeky and techy, and the creators of exciting coding adventures!<br/><br/>
      Ever wondered who's behind the scenes making all those mind-blowing tech events happen? That's us, the CSA! From organizing hackathons that redefine coding marathons to hosting talks that'll ignite fresh perspectives, we're here to keep the CS culture thriving on campus.<br/><br/>
      ...
      <h2 className="text-lg font-bold text-center text-white py-20"><b><i>"We are CSA, the champions of tech innovation and creators of a vibrant CS culture."</i></b></h2>
    </div>
  );
}
